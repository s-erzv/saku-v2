...
/**
 * GradientWaves — vendored from React Bits (reactbits.dev/backgrounds/gradient-waves),
 * TypeScript + Tailwind variant.

 * Kept byte-close to upstream so it can be re-pulled when it changes. The only local edit is the
 * `"use client"` directive above, which this app needs and a copy-paste snippet does not.
 *
 * Lint for this path is relaxed in `eslint.config.mjs` for the same reason: the React Compiler
 * rules fire on upstream's hook usage, and correcting them would fork the file, which is what
 * stops anyone ever pulling the fix when upstream ships one.
 */
