...
/** Installed app Home. This route never selects the browser dashboard. */
export default function AppHomePage() {
  const router = useRouter()
  const { user, isLoading, isAuthenticated } = useAuth()

  useEffect(() => {
    if (!isLoading && !isAuthenticated) router.replace("/get-started?view=app")
  ...
  return (
    <div className="min-h-dvh w-full min-w-0 max-w-lg bg-white font-sans relative mx-auto">
      ...
      <main className="relative z-10 mx-auto w-full min-w-0 max-w-lg space-y-[clamp(16px,4.69vw,24px)] px-[clamp(12px,3.13vw,16px)] py-2">
        <BalanceCardSection />
